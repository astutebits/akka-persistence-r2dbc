...
trait JournalLogic {
  ...
  lazy val reprSerializer = new PersistenceReprSerDe(SerializationExtension(system))
  ...
  def asyncWriteMessages(messages: Seq[AtomicWrite]): Future[Seq[Try[Unit]]] = {
    val asyncSerialized: Seq[Future[Try[Seq[JournalEntry]]]] = messages.map { atomicWrite =>
      ...
      val serialized = atomicWrite.payload.map(pr => reprSerializer.serialize(pr.withTimestamp(now)))
      Future.sequence(serialized).map(TryUtil.flatten)
    }

    val future = Source
      .future(Future.sequence(asyncSerialized))
  ...
  def asyncReplayMessages(persistenceId: String, fromSequenceNr: Long, toSequenceNr: Long, max: Long)(
    ...
    dao
      .fetchEvents(persistenceId, fromSequenceNr, toSequenceNr, max)
      .completionTimeout(replayTimeout)
      .mapAsync(1)(entry => reprSerializer.deserialize(entry).flatMap(Future.fromTry))
  ...
  def asyncReadHighestSequenceNr(persistenceId: String, fromSequenceNr: Long): Future[Long] = {
    def go(persistenceId: String, fromSequenceNr: Long) =
      dao.readHighestSequenceNr(persistenceId, fromSequenceNr).orElse(Source.single(0L)).runWith(Sink.last)
